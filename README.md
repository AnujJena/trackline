...
- **Floor Plan** — upload one or more floor plan images (per level/unit), or ask the assistant to sketch a schematic room layout (labeled room boxes, not a real image — Claude can't generate photographic or CAD-precision images, only text/data). Click anywhere on either type to drop a labeled pin (e.g. a punch item or RFI location); click a pin to remove it. Image upload has no AI drafting or spreadsheet import (it's not structured data); the AI schematic option only requires asking in chat.
